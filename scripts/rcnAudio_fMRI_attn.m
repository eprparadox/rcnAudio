function [] = rcnAudio_fMRI_attn(subject,block)
%
...
%%% load task map
load([subject '-attn_rcnAudio_task_map.mat'])
...
% get block type
block_type = cTrial_map(1).task_type;

if strcmp(block_type,'visual')
...
elseif strcmp(block_type,'audio')
    msg = 'You will hear a series of tones where one tone is out of place';
    DrawFormattedText(scr.textwin, msg, 'center', 'center', scr.white);
    Screen('CopyWindow', scr.textwin, scr.window, scr.rect, scr.rect);
    Screen('Flip', scr.window);
    Screen('FillRect', scr.textwin, scr.black, scr.rect);
    ...
    msg = 'Press 1 if you hear a tone with a different timing than the others';
    DrawFormattedText(scr.textwin, msg, 'center', 'center', scr.white);
    Screen('CopyWindow', scr.textwin, scr.window, scr.rect, scr.rect);
    Screen('Flip', scr.window);
    Screen('FillRect', scr.textwin, scr.black, scr.rect);
    ...
    msg = 'Press 2 if you hear a tone with a different pitch than the others';
    DrawFormattedText(scr.textwin, msg, 'center', 'center', scr.white);
    Screen('CopyWindow', scr.textwin, scr.window, scr.rect, scr.rect);
    Screen('Flip', scr.window);
    Screen('FillRect', scr.textwin, scr.black, scr.rect);
...
user_ans = zeros(1,length(cTrial_map));
response_time(t) = zeros(1,length(cTrial_map));

for t = 1:length(cTrial_map)
    
    visual_stims = cTrial_map(t).task_params.letters;
    %num_visual_stims = length(cTrial_map(t).task_params.letters);
    ...
    for v = 1:length(visual_stims)
        visual_start_time = GetSecs();
        if v == cTrial_map(t).task_params.targ;   
            DrawFormattedText(scr.textwin, visual_stim(v), 'center', 'center', scr.red);
        else
            DrawFormattedText(scr.textwin, visual_stim(v), 'center', 'center', scr.white);
        end
        Screen('CopyWindow', scr.textwin, scr.window, scr.rect, scr.rect);
        Screen('Flip', scr.window);
        Screen('FillRect', scr.textwin, scr.black, scr.rect);
            ...
            if key_not_pressed
                ...
                if keyIsDown
                    ...
                    if isequal(keypress,userkey_vischange)
                        user_ans(t) = 1;
                        response_time(t) = GetSecs() - visual_start_time;
